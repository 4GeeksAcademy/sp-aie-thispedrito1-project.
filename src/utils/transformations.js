// --- Facturación ---
export const calculateDenialRate = (claims) => {
    if (claims.length === 0)
...
export const denialRateByPayer = (claims) => {
...
export const flagHighDenialPayers = (claims, threshold = 8) => {
    const rates = denialRateByPayer(claims);
    return Object.keys(rates).filter(payer => rates[payer] > threshold);
};
// --- No-Show Cost ---
export const calculateNoShowCost = (appointments, location, weekEndingDate) => {
...
// --- CME Compliance ---
export const generateCMEReport = (clinicians, asOfDate) => {
