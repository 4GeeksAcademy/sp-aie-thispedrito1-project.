...
### Telemetría — `docs/telemetry/` (diseño, sin instrumentar)

Plan de diseño del hito de telemetría (rama `feat/telemetry-design-plan` sobre `feat/caching-optimisation`, 2026-08-12): `telemetry-plan.md` + `event-schemas.json`, sin código nuevo — es el catálogo que el equipo va a instrumentar en un hito posterior, no la instrumentación en sí. 23 eventos (5 obligatorios del CONTEXT + 18 de oportunidad identificados), cubriendo inventario, incidencias, proveedores, autenticación, rendimiento, errores de frontend y navegación, todos bajo un Event Envelope común (`eventId`, `timestamp`, `sessionId`, `userId`, `event_type`, `schemaVersion`, `requestId`, `properties`) y con allowlist de propiedades por evento (nada fuera de lista se emite). Decisión stream/batch justificada por urgencia de negocio, no preferencia técnica (7 eventos en stream: `stock_threshold_triggered`, `incident_created`, `supplier_status_changed`, `login_failed`, `password_reset_requested`, `api_error_response`, `frontend_error_captured`).

Brechas de esquema reales encontradas al revisar `services/api/` contra el CONTEXT, documentadas en la sección 7 del plan para resolver antes de instrumentar — no bloquean el diseño de hoy pero sí la implementación futura: `VALID_SUPPLY_CATEGORIES` en `models.py` (`ppe/wound_care/diagnostics/medications/consumables`) no coincide con las categorías del CONTEXT (`medication/ppe/consumable/equipment`) — el plan usa las del CONTEXT por requisito de la rúbrica; `MedicalSupply` no tiene campo `expiry_date` (lo necesita `supply_expiry_flagged`); no existe tabla de umbral mínimo configurable por clínica/producto (lo necesita `stock_threshold_triggered`); no existe ningún endpoint de edición directa de stock que rechazar (lo necesita `direct_stock_edit_rejected`, evento obligatorio del CONTEXT) — diseñado para dispararse el día que exista ese endpoint o guardia de repositorio. Decisión de minimización de datos deliberada: `incident_created` excluye `title`/`description` del allowlist (riesgo de PHI en texto libre pese al aviso ya existente en la UI de `/incidents/new`); los eventos de auth con un identificador de usuario (`login_failed`, `password_reset_requested`) lo emiten como hash HMAC-SHA256, nunca en texto plano.
